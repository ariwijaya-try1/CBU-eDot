...
- **✅ Fix DITERAPKAN (11 Agustus 2026):** `uom_levels[].id` sekarang **disamakan dengan `uom.id`** (id UOM master eSuite asli dari `GET /uom`, sama seperti yang dipakai di field `uom` -- misal `664219e2236dfcd5a400001a` buat "Units"), BUKAN hash generate sendiri lagi. `_generate_uom_level_id()` dihapus dari `product_sync_service.py`. **Ini masih pendekatan berbasis inferensi** (vendor belum kasih tau persis diganti jadi apa waktu mereka koreksi manual) -- **belum di-test end-to-end lewat bridge**. Kalau nanti user dapat detail persis dari vendor soal payload koreksi mereka dan ternyata beda dari `uom.id`, revisit lagi.
