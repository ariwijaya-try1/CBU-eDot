...
class CustomerPricelistMappingService:
    """
    Mass mapping Customer -> Pricelist di eSuite (field `price_list.id`, PDF
    section 9.8) -- BAGIAN TERAKHIR Task #4 (lihat sales_entities_gap.md
    project memory), melengkapi `sales.branchs[]`/`sales.salesmans[]` yang
    SUDAH ADA lewat CustomerSalesMappingService (pola/convention endpoint
    SAMA persis dengan itu -- lihat customer_sales_mapping_service.py).

    BEDA dari mapping sales (branch+salesman WAJIB bersamaan, keduanya
    array): field `price_list` di Customer itu 1 OBJECT TUNGGAL (bukan
    array) -- sesuai model bisnis "harga per toko" yang sudah dikonfirmasi
    ...
    lihat pricelist_sync_service.py) lewat EsuiteClient.find_by_external_codes()
    GENERIK (BUKAN pull loop manual seperti Branch) -- external_code
    Pricelist TERKONFIRMASI top-level (lihat GET /pricelists live yang user
    share 26 Agustus 2026: field `external_code` ada di level atas dokumen,
    BUKAN nested seperti Branch `basic_info.external_code`).
    ...
    def map_to_pricelist(self, external_codes: str, pricelist_external_code: str) -> dict:
        ...
        found = self.esuite.find_by_external_codes(ENTITY_PATH, {pricelist_code})
        record = found.get(pricelist_code)
        ...
        payload = [
            {"external_code": code, "price_list": price_list_payload}
            for code in codes
        ]
