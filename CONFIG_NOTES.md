...
**Dugaan baru (tahap 4, BELUM DIKONFIRMASI):** kemungkinan bug backend eSuite yang treat `0` sebagai falsy/"tidak ada value" (pola umum di backend JS/Node: `if (payload.cost) update...` -- `0` gagal truthy-check, dianggap sama dengan field tidak dikirim). Ini beda dari dugaan tahap 3 (computed 80% margin) -- itu sudah gugur. **Test yang perlu dilakukan user (belum dilakukan):** kirim `POST /product` manual (Swagger/Postman langsung, bukan lewat bridge) dengan `"cost": 1` (bukan 0) buat 1 produk yang sama, cek apakah kolom "Cost" di UI berubah jadi `Rp 1`. Kalau ya → konfirmasi falsy-skip, perlu tanya IT eSuite cara resmi clear ke 0 (bridge belum bisa dianggap final sebelum ini clear, JANGAN ubah kode berdasarkan dugaan ini dulu). Kalau tidak berubah sama sekali → balik ke dugaan lain, perlu digali lagi.
