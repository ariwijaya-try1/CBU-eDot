...
**`cost` -- REVISI 6 Agustus 2026, DIHAPUS dari payload.** Keputusan bisnis (instruksi boss): data cost/harga beli **tidak boleh dikirim** ke eSuite -- cuma `base_price` (harga jual) yang boleh. Sebelumnya field ini diisi dari `standard_price` Odoo dan ikut tersync (lihat riwayat 1247 produk di bawah, itu masih pakai versi lama yang include `cost`). `standard_price` tetap diambil dari Odoo di `get_products()` (tidak dihapus dari query, cuma tidak lagi dipetakan ke payload) -- kalau nanti ternyata field ini dibutuhkan lagi untuk keperluan lain, datanya sudah tersedia tanpa perlu ubah query.
