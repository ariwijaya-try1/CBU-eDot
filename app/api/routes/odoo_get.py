...
def get_odoo_product(
    limit: int | None = Query(default=50, ge=1, le=MAX_LIMIT, description=f"Batasi jumlah baris (default 50, maksimal {MAX_LIMIT})."),
...
@router.get("/odoo/uom")
def get_odoo_uom(
    limit: int | None = Query(default=100, ge=1, le=MAX_LIMIT),
...
def get_odoo_customer(
    limit: int | None = Query(default=50, ge=1, le=MAX_LIMIT),
    name: str | None = Query(default=None, description="OPSIONAL -- filter name ilike."),
):
    """
